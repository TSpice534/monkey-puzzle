...
content/survey.yaml (not a small fixture) — walks the full 12-step flow via
...
`profile_scope`, resolved via `profile_matrix` — the result page still
renders a reconstructed labelled 3x3 grid), `multi_range` choose-a-range
validation (backlog #0014), audience-aware wording (including before the
router is answered), and that the existing radar/fingerprint chart plus the
PDF/share/email routes still work against the new 12-question content shape.
...
# and content/survey.yaml (both audience tracks see all 12 — no `audience`
...
# ---------------------------------------------------------------------------
# Happy path — full 12-step flow, both audience tracks
# ---------------------------------------------------------------------------

def test_full_12_step_flow_individual_completes_and_classifies_via_profile_pair(client, db):
...
def test_full_12_step_flow_organisation_completes_and_classifies_via_profile_pair(client, db):
...
def test_high_scoring_answers_and_inventor_modifier_classify_as_innovators(client, db):
    ...
    client.post(f'/survey/{token}/step/{STEP_NEED_MOST}', data={'need_most': '0'})
    client.post(f'/survey/{token}/step/{STEP_HAVE_ENOUGH}', data={'have_enough': '0'})
    client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={'profile_approach': '2'})
    client.post(f'/survey/{token}/step/{STEP_PROFILE_SCOPE}', data={'profile_scope': '0'})  # -> inventor
    client.post(f'/survey/{token}/step/{STEP_TOPICS}', data={'topics': ['0', '1', '2']})
    client.post(f'/survey/{token}/step/{STEP_SUPPORT_TYPE}', data={'support_type': '0'})
    client.post(f'/survey/{token}/step/{STEP_TARGET_GROUPS}', data={'target_groups': '0'})
...
# ---------------------------------------------------------------------------
# Profile pair — mandatory to advance (steps 7 and 8 of the real survey,
# backlog #0017 Part B)
# ---------------------------------------------------------------------------

def test_profile_approach_step_cannot_be_skipped_on_the_real_survey(client, db):
    token = _start_new(client)
    _answer_up_to_grid(client, token, INDIVIDUAL)

    response = client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={})
...
def test_profile_scope_step_cannot_be_skipped_on_the_real_survey(client, db):
    token = _start_new(client)
    _answer_up_to_grid(client, token, INDIVIDUAL)
    client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={'profile_approach': '1'})

    response = client.post(f'/survey/{token}/step/{STEP_PROFILE_SCOPE}', data={})
...
def test_profile_approach_step_get_after_failed_advance_still_shows_step_seven(client):
    """A no-selection POST must not silently advance — GETing the same step
    afterwards should still be profile_approach, not step 8."""
    token = _start_new(client)
    _answer_up_to_grid(client, token, INDIVIDUAL)
    client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={})

    response = client.get(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}')
    assert response.status_code == 200
...
def test_why_reason_is_the_last_step_with_the_reworded_prompt(client):
    ...
    later shifted that final step number from 11 to 12 when the single
    profile_grid step became two."""
    token = _start_new(client)
    _answer_up_to_grid(client, token, INDIVIDUAL)
    client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={'profile_approach': '1'})
    client.post(f'/survey/{token}/step/{STEP_PROFILE_SCOPE}', data={'profile_scope': '1'})
    ...
    assert 'Question 12 of 12' in body
...
def test_profile_approach_step_with_malformed_value_is_treated_as_no_selection(client, db):
    """A garbage (non-integer) value for the profile_approach radio group
    must be rejected by the same mandatory guard as no selection at all —
    `_read_answer`'s `int(raw)` fails, returning None, which the mandatory
    profile-pair guard then treats identically to a skipped question — not
    silently accepted or a crashed route. (A `type: single` answer can't be
    malformed the old grid `"x,y"` way — this covers the single-value
    equivalent.)"""
    token = _start_new(client)
    _answer_up_to_grid(client, token, INDIVIDUAL)

    response = client.post(f'/survey/{token}/step/{STEP_PROFILE_APPROACH}', data={'profile_approach': 'garbage'})
