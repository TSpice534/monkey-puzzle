...
def test_render_innovation_curve_svg_has_no_text_elements():
    """Backlog #0040 — band-name labels moved out of the SVG entirely into
    the HTML `curve-legend` below the chart (`_result_innovation.html`), so
    they keep normal browser text scaling instead of shrinking with the
    responsive SVG downscale. The chart itself must carry no `<text>` at all."""
    from app.survey.charts import render_innovation_curve_svg
